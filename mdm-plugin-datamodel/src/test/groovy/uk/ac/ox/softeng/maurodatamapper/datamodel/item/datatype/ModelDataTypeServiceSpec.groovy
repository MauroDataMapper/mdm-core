...
@Slf4j
class ModelDataTypeServiceSpec extends BaseUnitSpec implements ServiceUnitTest<ModelDataTypeService> {
    ...
    def setup() {
        ...
        ModelDataType modelDataType = new ModelDataType(createdBy: StandardEmailAddress.UNIT_TEST, label: 'terminology type',
                                                        modelResourceDomainType: 'Terminology', modelResourceId: UUID.randomUUID())
        dataModel.addToDataTypes(modelDataType)
        dataModel.addToDataTypes(new ModelDataType(createdBy: StandardEmailAddress.UNIT_TEST, label: 'terminology type 2',
                                                   modelResourceDomainType: 'Terminology', modelResourceId: UUID.randomUUID()))

        dataModel.addToDataTypes(new ModelDataType(createdBy: StandardEmailAddress.UNIT_TEST, label: 'terminology type 3',
                                                   modelResourceDomainType: 'Terminology', modelResourceId: UUID.randomUUID()))
    ...
    void "test list"() {
        ...
        then:
        modelDataTypeList.size() == 2

        and:
        modelDataTypeList[0].label == 'terminology type 2'
    ...
    void "test delete"() {
        ...
        expect:
        service.count() == 3
        ModelDataType mdt = service.get(id)

        when:
        service.delete(mdt)

        then:
        ModelDataType.count() == 2
    }
}
