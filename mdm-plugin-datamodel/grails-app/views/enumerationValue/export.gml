...
EnumerationValue ev = enumerationValue as EnumerationValue

'mdm:enumerationValue' {
    ...
    if (ev.classifiers) {
        'mdm:classifiers' {
            ev.classifiers.each {cl ->
                layout '/classifier/ev.gml', classifier: cl, ns: 'mc'
            }
        }
    }
    if (ev.metadata) {
        'mdm:metadata' {
            ev.metadata.each {md ->
                layout '/metadata/ev.gml', metadata: md, ns: 'mc'
            }
        }
    }
    if (ev.annotations) {
        'mdm:annotations' {
            ev.annotations.each {ann ->
                layout '/annotation/ev.gml', annotation: ann, ns: 'mc'
            }
        }
    }
