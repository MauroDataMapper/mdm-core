...
class DataClassSpec extends ModelItemSpec<DataClass> implements DomainUnitTest<DataClass> {
    ...
    def setup() {
        ...
        dataSet.addToDataTypes(new ReferencePrimitiveType(createdByUser: admin, label: 'string'))
        dataSet.addToDataTypes(new ReferencePrimitiveType(createdByUser: admin, label: 'integer'))

        checkAndSave(dataSet)
        assert DataModel.count() == 1
        assert ReferenceDataType.count() == 2
    }
    ...
    void 'test adding dataelements'() {
        ...
        when:
        item = findById()
        ReferenceDataElement element = ReferenceDataElement.findByLabel('element1')
