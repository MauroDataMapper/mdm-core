...
class DataClassServiceSpec extends CatalogueItemServiceSpec implements ServiceUnitTest<DataClassService> {
    ...
    void 'test copying simple DataClass with simple data elements'() {
        ...
        when:
        copyModel.addToDataTypes(new ReferencePrimitiveType(createdByUser: admin, label: 'string'))
        copyModel.addToDataTypes(new ReferencePrimitiveType(createdByUser: editor, label: 'integer'))
    ...
    void 'test copying complex dataclass'() {
        ...
        when:
        ReferenceDataElement copiedElement = copiedParent.dataElements.find { it.label == 'parentel' }

        then:
        copiedElement
        copiedElement.referenceDataType == referenceType
    }
}
