...
abstract class DataBindReferenceDataModelImporterProviderServiceSpec<K extends DataBindReferenceDataModelImporterProviderService> extends BaseImportExportSpec {
    ...
    void 'test inc single primitive type data import'() {
        ...
        when:
        ReferenceDataType dataType = dm.dataTypes[0]

        then:
        dataType.instanceOf(ReferencePrimitiveType)
        dataType.label == 'openworld_tick'
        (dataType as ReferencePrimitiveType).units == 'mg'
    ...
    void 'test inc single enumeration type data import'() {
        ...
        then:
        dataType.instanceOf(ReferenceEnumerationType)
        dataType.label == 'Sex'
        ((ReferenceEnumerationType) dataType).referenceEnumerationValues.size() == 2
        ...
        when:
        ReferenceEnumerationValue val1 = ((ReferenceEnumerationType) dataType).referenceEnumerationValues.find {it.key == 'M'}
        ReferenceEnumerationValue val2 = ((ReferenceEnumerationType) dataType).referenceEnumerationValues.find {it.key == 'F'}
    ...
    void 'test inc single dataclass with data element type data import'() {
        ...
        when:
        ReferenceDataElement dataElement = dataClass.dataElements[0]
        ...
        and:
        dataElement.referenceDataType
        dataElement.referenceDataType.label == 'openworld_tick'
    ...
    void 'test inc dataclass with data element and reference datatype data import'() {
        ...
        and:
        dataElement.referenceDataType
        dataElement.referenceDataType.label == 'child'
    ...
    void 'test load datamodel with datatypes'() {
        ...
        dm.dataTypes.findAll {it.instanceOf(ReferencePrimitiveType)}.size() == 6
        dm.dataTypes.findAll {it.instanceOf(ReferenceEnumerationType)}.size() == 4

        and:
        ReferenceEnumerationType.count() == 5
        ReferencePrimitiveType.count() == 8
        ReferenceEnumerationValue.count() == 20
    }
