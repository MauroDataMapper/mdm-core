...
abstract class DataBindCodeSetImporterProviderService<T extends CodeSetFileImporterProviderServiceParameters> extends
    ...
    abstract CodeSet importCodeSet(User currentUser, byte[] content, boolean forceDefaultAuthority = true)
    ...
    CodeSet importModel(User currentUser, CodeSetFileImporterProviderServiceParameters params, boolean forceDefaultAuthority) {
        if (!currentUser) throw new ApiUnauthorizedException('FBIP01', 'User must be logged in to import model')
        if (params.importFile.fileContents.size() == 0) throw new ApiBadRequestException('FBIP02', 'Cannot import empty file')
        ...
        importCodeSet(currentUser, params.importFile.fileContents, forceDefaultAuthority)
    }

    CodeSet bindMapToCodeSet(User currentUser, Map codeSetMap, boolean forceDefaultAuthority = true) {
        ...
        codeSetService.checkImportedCodeSetAssociations(currentUser, codeSet, codeSetMap, forceDefaultAuthority)
