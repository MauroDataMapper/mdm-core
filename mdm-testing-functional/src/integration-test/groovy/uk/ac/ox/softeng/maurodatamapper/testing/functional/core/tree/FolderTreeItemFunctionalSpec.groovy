...
class FolderTreeItemFunctionalSpec extends TreeItemFunctionalSpec {
    ...
    @Override
    String getContainerParentOne() {
        getParentFolderId()
    }

    @Override
    String getContainerParentTwo() {
        getFolderId()
    }
  ...
  },
  {
    ...
    void 'AA : test available actions for folder inside versioned folder'() {
        ...
        when:
        localResponse = GET(vfId, Argument.listOf(Map))

        then:
        verifyResponse(OK, localResponse)

        and:
        localResponse.body().first().availableActions == [
            'createFolder',
            'createModel',
            'delete',
            'moveToFolder',
